...
class SeasonalTrendRegression:
    ...
    def __init__(self,
        ...
        self._t_mod = self._t % self._period

    @pycrt.enforce_precision('data')
    def fit(self,
            data: pyct.NDArray,
            *args,
            **kwargs) -> typ.Tuple[dict, pyct.NDArray, float]:
        r"""
        ...
        order = {comp: pycrt.coerce(xp.asarray(bspl.k)) for comp, bspl in bsplines.items()}
        ...
        lambda_ = pycrt.coerce(kwargs.pop("lambda", 1))
        ...
        # Form seasonal-trend design matrix

        if pycd.NDArrayInfo.from_obj(data) == pycd.NDArrayInfo.CUPY:
            sparse_module = pycd.SparseArrayInfo.CUPY_SPARSE
        elif pycd.NDArrayInfo.from_obj(data) == pycd.NDArrayInfo.DASK:
            sparse_module = pycd.SparseArrayInfo.PYDATA_SPARSE
        else:
            sparse_module = pycd.SparseArrayInfo.SCIPY_SPARSE

        spmats = prib.SeasonTrendBSplineDesignMat(samples=self._t, knots=knots, order=order, period=self._period,
                                                  sparse_module=sparse_module)
        FwdMat = spmats['sum']
        spmat_trend, spmat_season = spmats['trend'], spmats['season']
        if pycd.NDArrayInfo.from_obj(data) == pycd.NDArrayInfo.DASK:
            FwdMat = da.from_array(FwdMat)
            spmat_trend, spmat_season = da.from_array(spmat_trend), da.from_array(spmat_season)
        FwdOp = pyca.LinOp.from_array(FwdMat)
        _ = FwdOp.lipschitz(tol=1e-3)

        # Data fidelity loss
        if robust:
            loss = (1 / 2) * pyctest.L1Norm().moreau_envelope(mu).argshift(data)
            F = loss * FwdOp
        else:
        ...
        # Penalty terms
        bsi = prib.BSplineIntegral(interval=(0, self._period), knots=knots["season"], order=order["season"],
                                   extrapolate="periodic")
        G = prib.ZeroMeanSeas(order=order, knots=knots, bsi=bsi)
        H = pyctest.L1Norm()
        K = prib.SeasonalTrendBSpline2Inno(order=order, knots=knots)
        _ = K.lipschitz(tol=1e-3)
        penalty = H * K

        # Run optimisation algorithm
        max_outer_iterations = 10
        accuracy_hyperparameter = 1e-4
        fit_kwargs = dict(x0=xp.concatenate([coeffs["season"], coeffs["trend"]]).squeeze(),
                          z0=None, tau=kwargs.pop("tau", None), sigma=kwargs.pop("sigma", None),
                          rho=kwargs.pop("rho", None), tuning_strategy=kwargs.pop("tuning_strategy", 1),
                          stop_crit=kwargs.pop("stop_crit", None), mode=fit_mode,
                          track_objective=kwargs.pop("track_objective", False))
        print(f'Initial penalty: {lambda_}')
        if autotune:
            for i in range(max_outer_iterations):
                print(f'Major cycle {i + 1}/{max_outer_iterations}...')
                solver = pds.CondatVu(f=F, g=G, h=lambda_ * H, K=K, **kwargs)
                solver.fit(**fit_kwargs)
                x, z = solver.solution(which='primal'), solver.solution(which='dual')
                lambda_old = lambda_
                lambda_ = nb_of_parameters / (penalty(x) + 1)
                fit_kwargs.update(x0=x, z0=z)
                print(f'Penalty updated to value: {lambda_}')
                if np.abs(lambda_ - lambda_old) <= accuracy_hyperparameter * np.abs(lambda_old):
                    break
        else:
            solver = pds.CondatVu(f=F, g=G, h=lambda_ * H, K=K, **kwargs)
            solver.fit(**fit_kwargs)
        ...
        self.knots = knots
        self.order = order
        self.penalty_strength = lambda_
        self.data = data
        self.fitted_values = dict(sum=FwdOp(x), season=spmat_season @ self.coeffs["season"],
                                  trend=spmat_trend @ self.coeffs["trend"])
        self.residuals = {'seasonal': self.data - self.fitted_values['trend'],
                          'trend': self.data - self.fitted_values['seasonal'],
                          'sum': self.data - self.fitted_values['sum']}
        bsplines = dict(season=sci.BSpline(c=np.asarray(self.coeffs["season"]), t=knots['season'], k=order['season'],
                                           extrapolate="periodic"),
                        trend=sci.BSpline(c=np.asarray(self.coeffs["trend"]), t=knots['trend'], k=order['trend'],
                                          extrapolate=True))
        return bsplines, x, lambda_
